...
<form action="{{route('books.index')}}" method="POST" class="mt-10" enctype="multipart/form-data">
    ...
    <div class="mb-6">
        ...
        <label class="block mt-2">
            ...
            <input type="text" name="year" id="year" value="" class="block w-full px-3 py-2 mt-1 bg-white border rounded-md shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 sm:text-sm focus:ring-1" placeholder="1986" />
        </label>
        @error('year')
        <p class="mt-2 text-xs text-red-500">{{ $message }}</p>
        @enderror
