...
<div class="flex p-2 mb-10 -mt-12">
    ...
    <button class="flex p-2 text-white bg-gray-500 hover:bg-gray-600 rounded-xl">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          </svg> <a href="{{ route('authors.index') }}">All Authors</a>
    </button>
</div>
<form action="{{ route('authors.update',['author' => $author->id])}}" method="POST" class="mt-10" enctype="multipart/form-data">
    @csrf
    @method('PATCH')
    <div class="mb-6">
        <label class="block">
            ...
            <input type="text" name="name"  id="name" value="{{ $author->name }}" class="block w-full px-3 py-2 mt-1 bg-white border rounded-md shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 sm:text-sm focus:ring-1" placeholder="The Long Walk" />
        </label>
        @error('name')
